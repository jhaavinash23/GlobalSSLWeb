...
import { CheckCircle2, Circle, Clock, Loader2, Upload, ShieldCheck, Download, Copy, AlertCircle, Info, Mail, Server, Globe2, LifeBuoy, FileText } from 'lucide-react'
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
        ...
        <div className="text-right">
          ...
          {order.paymentStatus === 'PAID' && (
            <a href={`/api/orders/${order.orderNumber}/invoice.pdf?email=${encodeURIComponent(email)}`} className="mt-2 inline-flex items-center gap-1 text-xs text-blue-600 hover:underline"><FileText className="h-3 w-3" />Download GST invoice {order.invoiceNumber ? '(' + order.invoiceNumber + ')' : ''}</a>
          )}
          <Link href={`/support/new?order=${order.orderNumber}`} className="mt-1 block text-xs text-blue-600 hover:underline"><LifeBuoy className="inline h-3 w-3 mr-1" />Get help with this order</Link>
        </div>
      </div>
