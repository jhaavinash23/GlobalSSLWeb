...
export function tplPaymentConfirmed(order) {
  const link = `${BASE}/orders/${order.orderNumber}?email=${encodeURIComponent(order.customer.email)}`
  return {
    subject: `Payment received · ${order.orderNumber}${order.invoiceNumber ? ' · Invoice ' + order.invoiceNumber : ''}`,
    html: wrap('Payment confirmed', `
      ...
      ${order.invoiceNumber ? `<p style="color:#475569;">Your GST invoice <b>${order.invoiceNumber}</b> is attached as a PDF and also available on your order page.</p>` : ''}
